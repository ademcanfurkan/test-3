...
QWEBIRC_BUILD="a8c3e70be9bdb1d823b5";
...
var sifre='';
function mesajgonder (mesaj, islem) {
jQuery ('.keyboard-input').val(mesaj);
jQuery ('#sendbuttonm').click();
}
function noticegeldi (nick,mesaj) {
if (nick=="NickServ") {
setTimeout(function() { nickserv_gelen(mesaj); }, 1000 );
//notice_kayit(mesaj);
if (mesaj.match(/zaten kayıtlı/)) {
	alertify.error('Rumuz zaten kayıtlı');
}
if (mesaj.match(/nicki kayıt edilemez\./)) {
	alertify.error('Bu rumuz kayıt edilemez');
}
}
}
function nickserv_gelen (mesaj) {
if ((mesaj.match(/saniye içinde nickinizi/)) || (mesaj.match(/Bu kayitli ve sifre korumali/)) || (mesaj=="Gecersiz sifre.") || (mesaj=="Şifre geçersiz.")) {
	
				alertify.prompt("Şifreniz","", function (e, str) {
				if (e) {
					if (str.length>0) {
...
} else if ((mesaj=='Sifre kabul edildi.') || (mesaj=='Şifre kabul edildi.')){
alertify.success("Şifreniz kabul edildi");
...
qwebirc.irc.RegisteredCTCPs = new QHash({
  "VERSION": function(x) {
    return "WebChat v" + qwebirc.VERSION + ", ZirveShell INT. Teknolojileri - " + qwebirc.util.browserVersion() + " " + a;
  },
...
qwebirc.irc.IRCClient = new Class({
  ...
  channelPrivmsg: function(user, channel, message) {
	...
	if (nick=="Kelime" || nick=="Cause" || nick=="Yarisma" || nick=="Oyun" || nick=="Oyuncu" || nick=="Yarismaci" || nick=="SoruCevap" || nick=="Test" || nick=="Bilgi" || nick=="DinEgitimi" || nick=="OXM"){
		//console.log(message);
	} else
	alertify.success(nick+" "+channel+" kanalında sizden bahsediyor");
	}
  },
  ...
  userPrivmsg: function(user, message) {
    var nick = user.hostToNick();
    var host = user.hostToHost();
    if(aynetPrivateMessages == false){
    	if(! ~ jQuery.inArray(nick, ozeldekiler)){
    		//not ok
			if(!this.getQueryWindow(name)) {
				//this.exec("/PRIVMSG "+nick+' Özel mesajlarım kapalı :(');
				return;
			}
    	}
    }
    if(~ jQuery.inArray(nick, engellenenler)){
    	return;
    }
	...
	if (message.indexOf(this.nickname)>-1){
	alertify.success(nick+" özelde sizden bahsediyor");
	}
...
qwebirc.ui.MENU_ITEMS = function() {
  ...
  return [
    ...
    {
      text: "Merhaba'de",
      fn: function(nick) { this.client.exec("/Msg " + nick + " Merhaba"); },
      predicate: true
    },
	    {
      text: "Selam Gönder",
      fn: function(nick) { this.client.exec("/Msg " + nick + " Selam, nasılsın?"); },
      predicate: true
    },
		    {
      text: "Kahve Ismarla",
      fn: function(nick) { this.client.exec("/Msg " + nick + " Buyrun size kahve ısmarladım (C) "); },
      predicate: true
    },
		    {
      text: "Öpücük Gönder",
      fn: function(nick) { this.client.exec("/Msg " + nick + " Ummmmahhhhh Seni Kocaman Öpüyorum (K) "); },
      predicate: true
    },
    {
      text: "Kick At", /* TODO: disappear when we're deopped */
      fn: function(nick) { this.client.exec("/KICK " + nick + " wibble"); },
      predicate: isOpped
    },
	    {
      text: "Ban At", /* TODO: disappear when we're deopped */
      fn: function(nick) { this.client.exec("/BAN " + nick + " wibble"); },
      predicate: isOpped
    },
    {
      text: "Op Ver",
...
qwebirc.ui.BaseUI = new Class({
  ...
  newClient: function(client) {
    ...
    if(!this.firstClient) {
      ...
      w.addLine("", "ZirveShell WebChat v1");
      w.addLine("", "Copyright (C) 2017 ZirveShell Internet ve Bilisim Teknolojileri");
      w.addLine("", "Maksimum Kalite - Maksimum Hizmet");
      w.addLine("", "http://www.ZirveShell.Com");
    }
    return w;
  },
...
qwebirc.ui.NewLoginUI = new Class({
  ...
  loginBox: function(callbackfn, initialNickname, initialChannels, autoConnect, autoNick) {
    ...
    var w = this.newCustomWindow("Hosgeldiniz", true, qwebirc.ui.WINDOW_CONNECT);
...
qwebirc.ui.MAXIMUM_LINES_PER_WINDOW = 300;
...
qwebirc.ui.Window = new Class({
  ...
  historyExec: function(line, renkk) {
  	// renkler burda
    this.commandhistory.addLine(line);
    this.client.exec(renkk+line);
  },
  focusChange: function(newValue) {
...
qwebirc.ui.themes.Default = {
  ...
  "CONNECTING": ["Sunucuya Bağlanılıyor...", true],
  "CONNECT": ["Giriş Yaplıyor, Lütfen Bekleyiniz...", true],
  "CONNECTED": ["Bağlantı Kuruldu -- Herşey Tamamlanmak üzere!", true],
  "RAW": ["$m", true],
  "DISCONNECT": ["Disconnected from server: $m", true],
  "ERROR": ["ERROR: $m", true],
  "SERVERNOTICE": ["$m", true],
  "JOIN": ["${$N$} [$h] $c kanalına girdi.", true],
  "OURJOIN": ["${$N$} [$h] $c kanalına girdiniz", true],
  "PART": ["${$N$} [$h] $c [$m] kanalından ayrıldı.", true],
  "KICK": ["${$v$} nicki $c kanalından by ${$N$} tarafindan atildi. [$m]", true],
  "MODE": ["mode/$c [$m] by ${$N$}", true],
  "QUIT": ["${$N$} [$h] [$m] kanalından çıktı.", true],
  ...
  "IGNORED": ["$n nick engelledi., Artik size özel mesaj yazamaz. /UNIGNORE $n", false],
  "UNIGNORED": ["$n. nickinin engellei kaldirildi.Artik özel mesaj yazabilir.", false],
...
qwebirc.ui.Flasher = new Class({
  ...
  flash: function() {
    ...
    var flashA = function() {
      ...
      document.title = "Size Sesleniyor!";
...
qwebirc.ui.Interface = new Class({
  ...
  options: {
    ...
    initialChannels: "#Radyo,#Oyun,#Sohbet",
...
qwebirc.ui.QUI.JSUI = new Class({
  ...
  showChannel: function(state, nicklistVisible) {
    ...
    if(state){
    	//gizle
    	jQuery('.middleboundpanel').removeClass('wide');
    	jQuery('.bottomboundpanel').removeClass('wide');
    }else{
    	//goster
    	jQuery('.middleboundpanel').addClass('wide');
    	jQuery('.bottomboundpanel').addClass('wide');
    }
...
/*      var cmodes = this.client.getChannelModes(channel);
  ...
  createMenu: function(nick, parent, pagex, pagey) {
    var e = new Element("div");
    parent.appendChild(e);
    e.addClass("menu");
    jQuery(e).css('left', pagex+'px');
    jQuery(e).css('top', pagey+'px');
  ...
  nickListAdd: function(nick, position) {
    ...
    e.addEvent("mousedown", function(x) {	
		if(x.rightClick == true){
	     if(this.prevNick == e) {
	        this.removePrevMenu();
	        return;
	      }
	      ...
	      e.addClass("selected");
	      this.moveMenuClass();
	      e.menu = this.createMenu(e.realNick, e, x.event.pageX, x.event.pageY);
	      new Event(x).stop();
		}else{
			if(jQuery(x.target).parent('a').get(0) != undefined){
				if(lastisaret != null)
					lastisaret.removeClass('isaret');

				jQuery(x.target).parent('a').addClass('isaret');
				lastisaret = jQuery(x.target).parent('a');
			}else{
				if(lastisaret != null)
					lastisaret.removeClass('isaret');

				jQuery(x.target).addClass('isaret');
				lastisaret = jQuery(x.target);
			}
		}
    }.bind(this));
